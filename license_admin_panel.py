"""
License Admin Panel - Cloud MongoDB Atlas
Manage TasteParadise licenses from MongoDB Atlas
Author: Amrit Gaur
"""
...
class LicenseAdmin:
    """MongoDB Atlas License Management"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    print("  2. MongoDB Atlas is down")
                    print("  3. Firewall blocking connection")
                    print("  4. Network timeout")
                    print("\n📝 SOLUTION:")
                    print("  1. Go to MongoDB Atlas → Network Access")
                    print("  2. Click 'Add IP Address'")
                    print("  3. Click 'Add Current IP Address'")
                    print("  4. Wait 2-3 minutes for changes to deploy")
                    print("  5. Or temporarily add 0.0.0.0/0 for testing")
    ...
    def generate_license(self, customer_name, plan_type, duration_days, max_devices=1):
        ...
        try:
            ...
            license_doc = {
                ...
                'customer_name': customer_name,
                'customer': customer_name,  # For backward compatibility
                'plan': plan_type,
                'generated_date': datetime.now().isoformat(),
                'issued_date': datetime.now().isoformat(),  # For backward compatibility
                'expiry_date': license_data['expiry_date'],
                'max_devices': max_devices,
                'max_activations': max_devices,  # For backward compatibility
                'revoked': False,
                'activated': False,
                'machine_id': None,
                'hardware_id': None
...
def display_license(license_doc):
    ...
    machine_id = license_doc.get('machine_id') or license_doc.get('hardware_id')
...
def main():
    """Main admin panel"""
    print("\n" + "="*70)
    print("    🔐 TASTEPARADISE LICENSE ADMIN PANEL 🔐")
    print("="*70)
    ...
    while True:
        ...
        if choice == '1':
            ...
            print("\nPlan Types:")
            print("  1. FREE (7 days)")
            print("  2. TRIAL (30 days)")
            print("  3. BASIC (1 year)")
            print("  4. PRO (1 year)")
            print("  5. ENTERPRISE (10 years)")
            print("  6. CUSTOM (specify days)")
            
            plan_choice = input("\nSelect Plan (1-6): ").strip()
            ...
            if plan_choice == '6':
                # Custom duration
                custom_days = input("Enter duration in days: ").strip()
                try:
            ...
            if confirm == 'yes':
                if admin.revoke_license(license_key):
                    ...
                    print("   Customer's app will stop working on next check.")
                else:
            ...
            if admin.extend_license(license_key, days):
                ...
                updated_doc = admin.get_license(license_key)
                print(f"\n✅ License extended by {days} days!")
                print(f"📅 New expiry: {updated_doc['expiry_date'][:10]}")
            else:
